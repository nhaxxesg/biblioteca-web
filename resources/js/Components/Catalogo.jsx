...
const Catalog = () => {
    ...
    const filteredItems = items
        ...
        .filter(item => item.code.includes(searchTerm) || item.title.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const highlightMatch = (text) => {
        ...
        return (
            <>
                {text.slice(0, index)}
                <span className="bg-yellow-200">{text.slice(index, index + searchTerm.length)}</span>
        ...
        <div className="max-w-4xl mx-auto p-6 bg-[#f9f9f9] rounded-lg shadow-md">
            ...
            <div className="flex gap-4 mb-4 justify-center">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={() => handleStatusFilter('No Disponible')}
                        className={`px-4 py-2 border-b-2 transition-all duration-200 ${
                    ...
                    {filteredItems.length > 0 ? (
                        filteredItems.map((item) => (
                            <tr key={item.code} className="hover:bg-[#f2e7d5] transition duration-200 ease-in-out">
                                ...
                                <td className="py-2 px-5 border-b text-center font-serif">{highlightMatch(item.title)}</td>
                                ...
                                <td className={`py-2 px-5 border-b text-center font-serif ${item.status === 'Disponible' ? 'text-green-600' : 'text-red-600'}`}>
                                    {item.status}
                                </td>
                            </tr>
                        ))
